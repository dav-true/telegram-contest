...
dictionaryRus = io.open('D:\disk D\DataClusteringCompetition\\russianDic.txt', 'r', encoding='cp1251')
...
def detectLang(transListStr, extractedList):
    for i in range(len(getRusDictionaryList)):
        fall = re.findall(getRusDictionaryList[i], transListStr)
        if getRusDictionaryList[i] in transListStr:
            if 4 > len(getRusDictionaryList[i]) > 1:
                extractedList.append(getRusDictionaryList[i])
            elif len(getRusDictionaryList[i]) == 1:
                continue
            else:
                for l in range(len(fall)):
                    extractedList.append(getRusDictionaryList[i])
            ...
            try:
                try:
                    ...
                    for x in range(len(pageList)):
                        if x % 2 == 1:
                            transList.append(pageList[x])

                    transListStr = str(transList)
                    detectLang(transListStr, extractedList)
                    print(transList)
                    print(extractedList)
                except IndexError:
                    break
